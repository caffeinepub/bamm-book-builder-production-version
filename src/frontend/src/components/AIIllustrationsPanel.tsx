...
export function AIIllustrationsPanel({
  projectId,
  currentPageText,
  onInsertImage,
}: AIIllustrationsPanelProps) {
  ...
  const handleGenerateIllustration = async () => {
    ...
    console.log(
      "[AIIllustrationsPanel] Generating illustration with prompt:",
      prompt,
    );
  ...
  return (
    <Card
      className="relative z-[1001] overflow-visible bg-white"
      style={{ zIndex: 1001 }}
    >
      <CardHeader className="bg-gray-50">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardDescription>
              Generate custom images with Grok AI
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4 pt-6">
              ...
              >
                <CarouselContent>
                  {generatedImages.map((imageUrl, index) => (
                    <CarouselItem key={imageUrl || `img-${index}`}>
                      <div
        ...
        <div className="rounded-md border bg-muted/30 p-3">
          <p className="text-xs text-muted-foreground">
            <strong>Tip:</strong> Drag generated images directly onto the page
            canvas, or use the "Insert into Page" button. Images are optimized
            for 8.5×8.5" print format.
          </p>
        </div>
      </CardContent>
    </Card>
