...
export class StorageClient {
  ...
  public async putFile(
    ...
    this.hashCache.set(path, hashString);
    const url = await this.getDirectURL(path);
    return { path, hash: hashString, url };
  }

  public async listObjects(): Promise<{ path: string; hash: string }[]> {
    // Hash data is managed client-side; return empty list as backend does not store hashes
    return [];
  }
  ...
  public async getDirectURL(path: string): Promise<string> {
    ...
    const hash = this.hashCache.get(path);
    if (!hash) {
      throw new Error(
        `No hash found for path '${path}'. File may not have been uploaded in this session.`,
      );
    }

    // Validate hash format received from local cache
    validateHashFormat(hash, `getDirectURL for path '${path}'`);
